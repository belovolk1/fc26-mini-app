...
-- 3a) Политики tournaments: выборка, вставка, обновление, удаление (anon — приложение и скрипт симуляции)
...
-- 5) RPC для сетки и авто-тиков (убирают 404 в приложении)
...
BEGIN
  ...
  FOR v_round IN REVERSE v_rounds..1 LOOP
    FOR v_match_idx IN 0..(power(2, v_round) - 1) LOOP
      v_scheduled_start := v_tournament.tournament_start + ((v_rounds - v_round) * coalesce(v_tournament.round_duration_minutes, 30) || ' minutes')::interval;
      v_scheduled_end := v_scheduled_start + (coalesce(v_tournament.round_duration_minutes, 30) || ' minutes')::interval;
      IF v_round = v_rounds THEN
        ...
        VALUES (
          p_tournament_id, v_round, v_match_idx,
          v_players[v_match_idx * 2 + 1], v_players[v_match_idx * 2 + 2],
...
BEGIN
  ...
  IF v_m.round <= 1 THEN RETURN; END IF;

  v_next_round := v_m.round - 1;
  v_next_idx := v_m.match_index / 2;
  v_is_a := (v_m.match_index % 2);
...
BEGIN
  ...
  LOOP
    ...
    SET status = v_new_status, winner_id = v_winner_id,
...
BEGIN
  ...
  WHERE tournament_id = p_tournament_id AND round = 1 AND match_index = 0 AND winner_id IS NOT NULL;
